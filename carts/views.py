...
def _cart_id(request):  # underscore indicate its a provate function
...
def cart(request, total=0, quantity=0, cart_items=None):
    try:
        vat = 0
        grand_total = 0
        # getting the cart using the card_id present in the session:
        if request.user.is_authenticated:
...
#             cart=cart
#         )
#         # we need existing variation from db, current variation from product variation and item id from db
